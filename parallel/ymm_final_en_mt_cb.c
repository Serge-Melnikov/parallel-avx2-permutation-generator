 ...
 *   - Production-ready dynamic thread-safe Callback execution interface (PermCallback).
 *
 * @environment
...
/* Cross-compiler trailing zero count configuration */
#if defined(_MSC_VER)
    #include <intrin.h>
...
/* Define the signature for the user processing callback function */
typedef void (*PermCallback)(const U8* singleLane, I32 length, int thread_id, I64 thread_quanta);

/* Uncomment to enable verbose execution printouts */
...
/* Demonstration user callback that prints the generated permutations thread-safely */
void MyDemoCallback(const U8* singleLane, I32 length, int thread_id, I64 thread_quanta)
{
  ...
  for (I32 i = 0; i < length; i++) offset += sprintf(buf + offset, "%c", chars[singleLane[i]]);
  offset += sprintf(buf + offset, " | ");

  /* 2. Print the same permutation in reverse order (right-to-left) */
  for (I32 i = length - 1; i >= 0; i--) offset += sprintf(buf + offset, "%c", chars[singleLane[i]]);
  sprintf(buf + offset, "\n");

  /* Thread-safe console synchronization block to maintain output order */
...
/* Highly optimized ladder processing function capturing the full n-element permutations atomically */
static inline void ProcessExtendedLadder(I32 n_val, __m256i currentYMM, int thread_id, I64 thread_quanta, __m256i *localChecksum, PermCallback cb)
{
  ...
  /* CALLBACK EXECUTION: Trigger user routines for the base ladder layout */
  #ifdef HALF_PRINT_ENABLED
  if (cb)
  {
      ALIGN_32 U8 debugBuf[32];
      _mm256_store_si256((__m256i*)debugBuf, ladderYMM);
      cb(debugBuf, n_val + 1, thread_id, thread_quanta);      /* Lane A mapped to output channel */
      cb(debugBuf + 16, n_val + 1, thread_id, thread_quanta); /* Lane B mapped to output channel */
  }
  #else
  ...
  for (I64 step = 0; step < n_val; step++)
  {
     ...
     /* CALLBACK EXECUTION: Trigger user routines for every intermediate sweep step */
     #ifdef HALF_PRINT_ENABLED
     if (cb)
     {
         ALIGN_32 U8 debugBuf[32];
         _mm256_store_si256((__m256i*)debugBuf, ladderYMM);
         cb(debugBuf, n_val + 1, thread_id, thread_quanta);
         cb(debugBuf + 16, n_val + 1, thread_id, thread_quanta);
     }
     #endif
  }
...
void GenPermYMM_Half_Parallel(I32 n_val, int thread_id, I64 threadTargetPerm, I64 startNumA, I64 startNumB, PermCallback cb)
{
  ...
  I64 totalCount = 0;
  ProcessExtendedLadder(n_val, currentYMM, thread_id, totalCount, &threadLocalChecksum, cb);
  totalCount++;
...
int main(int argc, char* argv[])
{
  ...
  /* Select dynamic pointer assignment based on verbose compilation switches */
  PermCallback userCallback = NULL;
  #ifdef HALF_PRINT_ENABLED
  userCallback = MyDemoCallback;
  #endif
  ...
  #pragma omp parallel
  {
      ...
      {
          threadTarget = totalSubSpace - startNumA;
      }

      GenPermYMM_Half_Parallel((I32)n, tid, threadTarget, startNumA, startNumB, userCallback);
  }
